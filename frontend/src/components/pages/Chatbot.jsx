...
export default function Chatbot() {
  ...
  const renderLink = (url, label) => {
    return `</br><a href="${url || '#'}" class="text-[#202020] ${url ? 'hover:text-green-900 hover:underline' : 'pointer-events-none opacity-50'}" target="_blank" rel="noopener noreferrer">[${label}]</a>`;
  }

  const method = (data) => {
    return data ? `<br/><br/><strong>📋 배출 방법</strong>${data.replace(/\n/g, '<br/>')}` : null;
  }

  const renderItem = (icon, label, value) => {
    return value
      ? `<span class="mt-4 mb-4">&nbsp;${icon} ${label}: ${value.replace(/\n/g, '<br/>')}</span><br/>`
      : null;
  };
  ...
  const localInfo = (gov, type) => {
    if (gov.wasteType === type) {
      ...
      return `
        <br/><strong>🗂️ ${typeLabel} 수거 정보</strong><br/>
        ${[
  ...
  const getDetailMessages = (detail, parentId) => {
    return detail.flatMap((item) => {
      ...
      const basicMessage = `<br/><strong class="text-[#202020]"><br/>🚨 참고 부탁드립니다!</strong><br/>정보가 변경될 수 있으니, 정확한 사항은 홈페이지에서 확인해 주세요.`;
      ...
      return item.localGovern
        ...
        `;
          ...
          if (gov.wasteType == "GENERAL" && parentId == 43) { data += `${renderLink(gov.generalUrl, "일반쓰레기 수거 정보 보기")}`; }
            ...
            return {
              type: "AI",
              text: `<strong>📍${local} 수거 정보</strong><br/>
              ${infos}
              <br/>
